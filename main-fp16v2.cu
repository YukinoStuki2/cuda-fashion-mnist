...
// 新增：选择每个 block 一次计算的输出通道数
#ifndef OC_TILE
#define OC_TILE 2   // C2_OUT=16，2 或 4 都可；2 更稳妥
#endif
...
// 常量内存（conv 权重/偏置 + fc3 偏置）// 新增：把 FC1/FC2 的 bias 放常量内存
...
// IF1 + pool1 融合：输入/输出都用 half，膜电位 v1 仍用 float
__launch_bounds__(256, 2)
...
// 替换 conv2 融合核：去掉原 __launch_bounds__(16,16)，并用 half2 向量化搬运
__global__ void conv2_if_pool2_fused_const_smem_flat(
    ...
    const int tid    = threadIdx.y * blockDim.x + threadIdx.x; // 线性 tid
    const int tcount = blockDim.x * blockDim.y;                 // 线程数
    const int plane  = H_in * W_in;                             // 144
    const int plane2 = plane >> 1;                              // 72

    for (int ic = 0; ic < C2_IN; ++ic) {
        // 以 half2 向量化方式搬运到 shared，并转为 float
        ...
        for (int idx2 = tid; idx2 < plane2; idx2 += tcount) {
            __half2 h2 = gptr2[idx2];           // 连续两个 half
            float2 f2 = __half22float2(h2);
            int i0 = (idx2 << 1);
            s_x[i0 + 0] = f2.x;
            s_x[i0 + 1] = f2.y;
        }
        // 若 plane 为奇数则收尾（此处 144 为偶数，可省，但保守写法不伤性能）
        if ((plane & 1) && tid == 0) {
            s_x[plane - 1] = __half2float(gptr[plane - 1]);
        }
        __syncthreads();
...
// 新增或替换：conv2 + IF2 + pool2 融合核，half2 读入 + OC_TILE=2 复用 shared x
__global__ void conv2_if_pool2_fused_const_smem_flat_oc2(
        const __half* __restrict__ x_p1_h, // [N, C2_IN, 12,12] half
        float* __restrict__ v2,            // [N, C2_OUT, 8,8] float
        float* __restrict__ y_flat,        // [N, 16*4*4] float 直写
        int N, int H_in, int W_in, float v_th
){
    extern __shared__ float s_x[];             // 12*12 floats
    const int Hout = H_in - K2 + 1;            // 8
    ...
    int oc_base  = (z % (C2_OUT/OC_TILE)) * OC_TILE;  // 本 block 的起始输出通道

    int py0 = oyp * 2;
    int px0 = oxp * 2;

    // 为每个 oc 保留 4 个累积器（2x2 池化窗口的 4 个点）
...
#pragma unroll
    ...
    // 写回 v2，做 IF+pool2（max over 2x2），并写扁平化输出
#pragma unroll
    for (int t = 0; t < OC_TILE; ++t) {
        int oc = oc_base + t;
...
// 新增：FC1/FC2 专用 kernel（bias 从常量内存取）
// 参数与原来的 linear_if_fp16W_cached_kernel 基本一致，但去掉 b 参数
template<int I_MAX>
__launch_bounds__(256, 2)
...
// 每个 block 固定一个输出通道 ox，block 内多样本 n 共享该通道的权重行（FC1/FC2 用）
template<int I_MAX>
__launch_bounds__(256, 2)
__global__ void linear_if_fp16W_cached_kernel(
    ...
    extern __shared__ float shW[]; // I floats
    const int ox = blockIdx.x; // 固定输出通道
    int n  = blockIdx.y * blockDim.y + threadIdx.y;
    if (ox >= O || n >= N) return;
    ...
    // 向量化处理 x（shW 用标量读取，避免 shared 上的对齐顾虑）
    int i = 0;
#pragma unroll
    for (; i + 4 <= I; i += 4) {
        float4 xv4 = reinterpret_cast<const float4*>(xrow)[i >> 2];
        float w0 = shW[i+0], w1 = shW[i+1], w2 = shW[i+2], w3 = shW[i+3];
        acc = fmaf(xv4.x, w0, acc);
        acc = fmaf(xv4.y, w1, acc);
        acc = fmaf(xv4.z, w2, acc);
        acc = fmaf(xv4.w, w3, acc);
    }
    for (; i < I; ++i) acc = fmaf(xrow[i], shW[i], acc);
...
// ================== 推理主流程 ==================
// scnn_inference 的关键改动：
// - conv1 输出转换为 half 并缓存
// - if1/pool1 输入/输出都用 half
// - conv2 融合核直接写 d_flat，删除 flatten kernel & d_p2
std::vector<int> scnn_inference(
    ...
    // FC1/FC2 半精度权重
    __half *d_fc1_w_h=nullptr, *d_fc2_w_h=nullptr;
    ...
    cudaFuncSetCacheConfig(conv2_if_pool2_fused_const_smem_flat, cudaFuncCachePreferL1);

    // 主机端 pinned 双缓冲 + streams
    ...
    dim3 blk2d(16, 16, 1);
    dim3 blkP1(8, 8, 1);
    dim3 blkF(std::min(8, P2_W), std::min(8, P2_H), 1);
    const int tileN = 128;           // V100 上更稳
    ...
    for (int bi = 0; bi < total_batches; ++bi) {
        ...
        for (int t = 0; t < T; ++t) {
            ...
            // conv2 + IF2 + pool2（half 输入，直接写扁平化）— 使用 OC_TILE=2 版本
            {
