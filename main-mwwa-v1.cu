...
#endif
#ifndef USE_WMMA
#define USE_WMMA 1   // 运行时仍会检查是否在 >=sm_70 上，否则回退
#endif
...
// FC2 WMMA 维度（K=120 需 pad 到 128；N=84 pad 到 96）
...
// 放在文件头部 includes 后、任何 WMMA 内核定义之前
#if USE_WMMA
// 统一 WMMA 使用的半精度类型。部分环境下 WMMA 需要的是 `half` 而非 `__half`
using half = __half;
#include <mma.h>
#endif
...
// conv2 + IF2 + pool2 融合（half2 搬运 x 到 shared），一次算 OC_TILE 个输出通道，并直接写扁平化输出
__global__ void conv2_if_pool2_fused_const_smem_flat_oc2(
        ...
        float* __restrict__ y_flat,        // [N, 16*4*4] float
...
// 为了严谨，这里提供一个一步到位的转置+pad kernel（覆盖上面的简化）
__launch_bounds__(256, 2)
__global__ void fc1_transpose_pad_colmajor_v2(
        const __half* __restrict__ W_h, // [O, I] row-major
        __half* __restrict__ Wt_col,    // [I, Npad] col-major (ld = Npad)
...
// FC2: in W_h[O=84, I=120] -> out Wt[Kpad=128, Npad=96]（Kpad/Npad pad 为 0）列主
__launch_bounds__(256, 2)
__global__ void fc2_transpose_pad_colmajor(
    ...
    int k = blockIdx.y * blockDim.y + threadIdx.y; // 0..Kpad-1（行）
    int n = blockIdx.x * blockDim.x + threadIdx.x; // 0..Npad-1（列）
...
// 将输入浮点阵转 half（FC1 输入：N x 256，无需 pad）
__launch_bounds__(256, 2)
__global__ void pack_fc1_in_half(const float* __restrict__ in, __half* __restrict__ out, int N, int K)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int total = N * K;
    if (idx < total) out[idx] = __float2half(in[idx]);
}

// 将 FC2 输入浮点（N x 120）拷贝到 half（N x 128），尾部 pad 为 0
__launch_bounds__(256, 2)
__global__ void pack_fc2_in_half_pad128(const float* __restrict__ in, __half* __restrict__ out, int N, int K, int Kpad)
{
    int n  = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    float v = 0.f;
    if (k < K) v = in[n * K + k];
    out[n * Kpad + k] = __float2half(v);
}

// FC1 WMMA 内核：sm_70+ 用 WMMA；否则用纯 FMA 回退，确保结果正确
__global__ void fc1_wmma_if_kernel(
        const half* __restrict__ x_h,    // [N, 256] row-major, ldA = 256
        const half* __restrict__ wT_h,   // [256, 128] col-major, ldB = 256（列主的行数）
    ...
    for (int k0 = 0; k0 < K; k0 += WMMA_K) {
        ...
        const half* Bptr = wT_h + n0 * K + k0;  // ldB = K（列主 stride = 行数）
...
#else
    // 纯 FMA 回退：每线程负责瓦片内 8 个元素（与上面相同的划分）
    const int lane = threadIdx.x & 31;
    for (int i = lane; i < WMMA_M * WMMA_N; i += 32) {
        const int r = i / WMMA_N;
        const int c = i % WMMA_N;
...
#pragma unroll 1
            for (int k = 0; k < K; ++k) {
                float a = __half2float(x_h[a_row_off + k]);
...
// FC2 WMMA 内核：sm_70+ 用 WMMA；否则用纯 FMA 回退，确保结果正确
...
std::vector<int> scnn_inference(
    ...
    float *d_flat=nullptr;  // conv2 fused 写扁平化（float）
    float *d_if3_v=nullptr, *d_if3_s=nullptr;
    float *d_if4_v=nullptr, *d_if4_s=nullptr;
    float *d_acc=nullptr;
    int   *d_pred=nullptr;

    // WMMA 额外缓冲（权重转置+pad；输入 half 化/Pad）
    __half *d_flat_h=nullptr;              // [BATCH, 256]
    __half *d_if3_s_pad_h=nullptr;         // [BATCH, 128]
    ...
    for (int bi = 0; bi < total_batches; ++bi) {
        ...
        for (int t = 0; t < T; ++t) {
            ...
            // conv2 + IF2 + pool2（half 输入，直接写扁平化）
            {
                ...
                for (int b0 = 0; b0 < curB; b0 += step) {
                    ...
                    float*        yflat = d_flat   + (size_t)b0 * FLAT;
            ...
            if (use_wmma) {
#if USE_WMMA
                // FC1: d_flat(float N x 256) -> d_flat_h(half) 然后 WMMA
                // FC1 WMMA 启动
                {
                    int tot = curB * FC1_IN;
                    float_to_half_kernel<<<div_up(tot,256), 256, 0, stream_comp>>>(d_flat, d_flat_h, tot);
                    checkCudaErrors(cudaGetLastError());

                    dim3 grid(FC1_NPAD / WMMA_N, div_up(curB, WMMA_M));
                    dim3 block(32); // 单 warp
                    fc1_wmma_if_kernel<<<grid, block, 0, stream_comp>>>(d_flat_h, d_fc1_wT_h, d_if3_v, d_if3_s, curB);
                    checkCudaErrors(cudaGetLastError());
                }
                ...
                {
                    dim3 blk(32, 8);
                    dim3 grd(div_up(FC2_KPAD, blk.x), div_up(curB, blk.y));
                    pack_fc2_in_half_pad128<<<grd, blk, 0, stream_comp>>>(d_if3_s, d_if3_s_pad_h, curB, FC2_K, FC2_KPAD);
                    checkCudaErrors(cudaGetLastError());

                    dim3 grid(FC2_NPAD / WMMA_N, div_up(curB, WMMA_M));
                    dim3 block(32); // 单 warp
                    fc2_wmma_if_kernel<<<grid, block, 0, stream_comp>>>(d_if3_s_pad_h, d_fc2_wT_h, d_if4_v, d_if4_s, curB);
                    checkCudaErrors(cudaGetLastError());
                }
#endif
            } else {
                // 回退：半精度权重 + shared 缓存 GEMV
                {
            ...
            // FC3 累加（保持 float 权重，稳健）
            {
                dim3 blk(32, 8);
                dim3 grd(div_up(FC3_OUT, blk.x), div_up(curB, blk.y));
                fc3_accum_kernel_floatW<<<grd, blk, 0, stream_comp>>>(d_if4_s, d_fc3_w, d_acc, curB);
                checkCudaErrors(cudaGetLastError());
            }
        } // end T
