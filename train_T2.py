...
WEIGHT_DECAY = 1e-4
BATCH_SIZE = 512
EPOCHS = 160          # 或 180，注意 2 小时限制，160 一般够
T_TIMESTEPS = 2       # 一定要和推理一致

LABEL_SMOOTH_MAX = 0.01   # 比原来小很多
WARMUP_EPOCHS = 5
EMA_DECAY = 0.9995
SWA_LAST_EPOCHS = 30      # 稍微长一点
CLIP_NORM = 10.0
...
class SCNN(nn.Module):
    def __init__(self, T: int):
        ...
        # conv1: 1 -> 8（保持不变）
        self.conv1 = layer.Conv2d(1, 8, 5, bias=True)
        self.if1 = neuron.IFNode()   # 阈值回到默认，更合规
        self.pool1 = layer.MaxPool2d(2, 2)

        # conv2: 8 -> 24（缩一点宽度）
        self.conv2 = layer.Conv2d(8, 24, 5, bias=True)
        self.if2 = neuron.IFNode()
        self.pool2 = layer.MaxPool2d(2, 2)

        self.flatten = layer.Flatten()
        # flatten: 24 * 4 * 4 = 384
        self.fc1 = layer.Linear(24 * 4 * 4, 160, bias=True)
        self.if3 = neuron.IFNode()

        self.fc2 = layer.Linear(160, 96, bias=True)
        self.if4 = neuron.IFNode()

        self.fc3 = layer.Linear(96, 10, bias=True)
...
def main():
    ...
    # 和作业给的模板保持一致：直接把十个 txt 导出到脚本所在目录
    best_dir = script_dir
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch < int(EPOCHS * 0.5):
            smooth = LABEL_SMOOTH_MAX  # 比如 0.01
        else:
            smooth = 0.0
