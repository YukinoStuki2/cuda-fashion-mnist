...
__global__ void if1_pool1_fused_kernel(
        ...
        float* __restrict__ y_p1,       // [N, C1_OUT, 12, 12]
        int N,
...
// conv2 + IF2 + pool2 融合（shared-memory tiling + PTX FMA）
...
std::vector<int> scnn_inference(
    ...
    float *d_if1_v=nullptr, *d_p1=nullptr;
    ...
    const int tileN = 128; // 每 block 处理的样本行数
    dim3 blkLinear(1, tileN);  // FC（每 block 固定一个 ox）
    ...
    for (int bi = 0; bi < total_batches; ++bi) {
        ...
        for (int t = 0; t < T; ++t) {
            ...
            // conv2 + IF2 + pool2（shared memory 融合核）
            {
                ...
                for (int b0 = 0; b0 < curB; b0 += step) {
                    ...
                    size_t smem_bytes = (size_t)P1_H * P1_W * sizeof(float); // 576 B
            ...
            // fc1 + IF3（半精度权重 + shared 缓存；每 block 固定一个 ox）
            {
                dim3 grd(FC1_OUT, div_up(curB, tileN));
                size_t shmem = FC1_IN * sizeof(float);
                linear_if_fp16W_cached_kernel<FC1_IN><<<grd, blkLinear, shmem, stream_comp>>>(
                        d_flat, d_fc1_w_h, d_fc1_b, d_if3_v, d_if3_s, curB, FC1_IN, FC1_OUT, 1.0f);
                checkCudaErrors(cudaGetLastError());
            }

            // fc2 + IF4（半精度权重 + shared 缓存；每 block 固定一个 ox）
            {
                dim3 grd(FC2_OUT, div_up(curB, tileN));
                size_t shmem = FC2_IN * sizeof(float);
                linear_if_fp16W_cached_kernel<FC2_IN><<<grd, blkLinear, shmem, stream_comp>>>(
                        d_if3_s, d_fc2_w_h, d_fc2_b, d_if4_v, d_if4_s, curB, FC2_IN, FC2_OUT, 1.0f);
                checkCudaErrors(cudaGetLastError());
            }
